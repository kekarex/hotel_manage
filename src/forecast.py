"""!
@file forecast.py
@brief Модуль, реализующий расчет прогнозируемых данных.
"""
...
class Forecast:
    """!
    @brief Класс для расчёта прогнозных данных и оценки точности.

    Этот класс предоставляет методы для вычисления скользящей средней,
    прогнозирования значений и оценки ошибок прогноза.
    """
    ...
    def moving_average(data: List[float], n: int) -> List[float]:
        ...
        @param data Список числовых данных временного ряда.
        @param n Размер окна скользящей средней.
        @return Список значений скользящей средней. Пустой список, если данных недостаточно.
        """
        if len(data) < n:
            return []
        moving_averages = []
    ...
    def forecast_values(data: List[float], n: int, periods: int) -> List[float]:
        ...
        Использует метод скользящей средней и линейную экстраполяцию для прогнозирования.
        @param data Список исторических данных.
        @param n Размер окна скользящей средней.
        @param periods Количество периодов для прогноза.
        @return Список прогнозируемых значений. Пустой список, если данных недостаточно.
        """
        if len(data) < n:
            return []
        forecasts = []
    ...
    def calculate_errors(actual: List[float], predicted: List[float]) -> Tuple[float, float, float]:
        ...
        Вычисляет среднюю абсолютную ошибку (MAE), среднюю квадратическую ошибку (RMSE)
        и среднюю относительную ошибку (MRE).
        @param actual Список фактических значений.
        @param predicted Список прогнозируемых значений.
        @return Кортеж с тремя значениями: MAE, RMSE, MRE. Возвращает (0.0, 0.0, 0.0), если данные некорректны.
        """
    ...
    def interpret_accuracy(mean_relative_error: float) -> str:
        ...
        Оценивает точность на основе средней относительной ошибки (MRE).
        @param mean_relative_error Средняя относительная ошибка в процентах.
        @return Строковое описание точности: "Высокая", "Хорошая", "Удовлетворительная" или "Неудовлетворительная".
        """
