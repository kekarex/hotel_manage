...
class BookingDialog(QDialog):
    ...
    def __init__(self, db, booking=None, user_id=None):
        ...
        @param user_id Идентификатор пользователя, создающего бронирование
                       (опционально).
        """
        super().__init__()
        self.db = db
        ...
        self.setWindowTitle("Новое бронирование" if not booking
                           else "Редактирование бронирования")
        ...
        self.status.addItems(["reserved", "checked_in", "checked_out",
                            "cancelled"])
    ...
    def load_services(self):
        ...
        try:
            ...
            self.db.cursor.execute(
                "SELECT id, name, price FROM services WHERE is_active = 1"
            )
            ...
            for service in services:
                ...
                checkbox.stateChanged.connect(
                    lambda state, sb=quantity_spinbox: sb.setEnabled(state == 2)
                )
                ...
                self.services_widgets.append((service_id, checkbox,
                                            quantity_spinbox))
    ...
    def load_booking_data(self):
        ...
        self.room_number.setCurrentIndex(
            self.room_number.findData(self.booking[1])
        )

        try:
            self.db.ensure_connection()
            self.db.cursor.execute(
                "SELECT service_id, quantity FROM booking_services "
                "WHERE booking_id = ?",
                (self.booking[0],),
            )
    ...
    def load_available_rooms(self):
        ...
        logging.info(f"Загрузка номеров: type={room_type}, check_in={check_in}, "
        ...
        try:
            ...
            QMessageBox.warning(self, "Ошибка", f"Не удалось загрузить номера: "
                                    f"{str(e)}")
    ...
    def calculate_price(self):
        ...
        service_ids = [(sid, sb.value()) for sid, cb, sb in self.services_widgets
                      if cb.isChecked() and sb.value() > 0]

        logging.info(f"Расчет стоимости: room_id={room_id}, check_in={check_in}, "
                     f"check_out={check_out}, services={service_ids}")
        ...
        try:
            total_price = self.db.calculate_total_price(room_id, check_in,
                                                      check_out, guest_email,
                                                      service_ids)
    ...
    def save_booking(self):
        ...
        logging.info(f"Сохранение брони: room_id={room_id}, "
                     f"guest_name={guest_name}, total_price={total_price_text}")
        ...
        if not total_price_text or float(total_price_text) <= 0:
            ...
            QMessageBox.warning(self, "Ошибка",
                               "Рассчитайте стоимость перед сохранением")
            return
        ...
        try:
            ...
            if self.booking:
                self.db.cursor.execute(
                    ...
                    (guest_name, guest_phone, guest_email, check_in, check_out,
                     adults, children, room_id, status, total_price,
                     self.booking[0]),
                )
                self.db.cursor.execute(
                ...
                self.db.cursor.execute(
                    ...
                    INSERT INTO bookings (room_id, guest_name, guest_phone,
                                         guest_email, check_in_date,
                                         check_out_date, adults, children,
                                         status, total_price, created_by)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (room_id, guest_name, guest_phone, guest_email, check_in,
                     check_out, adults, children, status, total_price,
                     self.user_id),
                )
            ...
            self.db.conn.commit()
            QMessageBox.information(self, "Успех", "Бронирование успешно "
                                     "сохранено")
            self.accept()
        except sqlite3.Error as e:
            logging.error(f"Ошибка сохранения бронирования: {e}")
            QMessageBox.warning(self, "Ошибка",
                               f"Не удалось сохранить бронирование: {str(e)}")