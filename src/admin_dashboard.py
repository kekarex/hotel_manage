...
@brief Модуль, реализующий панель администратора для управления операциями отеля.
"""
...
class AdminDashboard(QMainWindow):
    """
    @brief Класс панели администратора для управления операциями отеля.
    """
    ...
    def load_bookings_data(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Ошибка", "Не удалось загрузить данные бронирований"
            )
    ...
    def edit_booking(self):
        ...
        if selected_row == -1:
            QMessageBox.warning(
                self, "Ошибка", "Выберите бронирование для редактирования"
            )
            return
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Ошибка",
                f"Не удалось загрузить данные бронирования: {str(e)}",
            )
    ...
    def cancel_booking(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.warning(
                    self, "Ошибка", f"Не удалось отменить бронирование: {str(e)}"
                )

    def assign_rooms(self):
        """
        @brief Распределение номеров (заглушка).
        @note Функция пока не реализована и будет добавлена в будущих версиях.
        """
        ...
        try:
            ...
            self.db.cursor.execute(
                ...
                SELECT r.id, r.number, r.type, r.status,
                       CASE WHEN b.guest_name IS NOT NULL THEN b.guest_name
                            ELSE '' END as guest,
                       r.description
                FROM rooms r
    ...
    def edit_room_status(self):
        ...
        if selected_row == -1:
            QMessageBox.warning(
                self, "Ошибка", "Выберите номер для изменения статуса"
            )
            return
        ...
        if dialog.exec_() == dialog.Accepted:
            ...
            try:
                ...
                QMessageBox.warning(
                    self, "Ошибка", f"Не удалось обновить статус номера: {str(e)}"
                )
        ...
        try:
            ...
            self.db.cursor.execute(
                ...
                SELECT c.id, c.full_name, c.email, c.phone, c.discount,
                       (SELECT COUNT(*) FROM bookings WHERE guest_email = c.email)
                       as bookings_count
                FROM clients c
    ...
    def edit_discount(self):
        ...
        if selected_row == -1:
            QMessageBox.warning(
                self, "Ошибка", "Выберите клиента для изменения скидки"
            )
            return
        ...
        if dialog.exec_() == dialog.Accepted:
            ...
            try:
                self.db.ensure_connection()
                self.db.cursor.execute(
                    "UPDATE clients SET discount=? WHERE id=?",
                    (new_discount, client_id),
                )
                self.db.conn.commit()
                self.load_clients_data()
            except sqlite3.Error as e:
                QMessageBox.warning(
                    self, "Ошибка", f"Не удалось обновить скидку: {str(e)}"
                )

    def init_reports(self):
        """
        @brief Инициализация раздела финансовых отчетов.
        """
        widget = QWidget()
        layout = QVBoxLayout()
        widget.setLayout(layout)
        ...
        self.report_type.addItems(
            ["Доходы", "Расходы", "Количество бронирований", "Популярность услуг"]
        )
    ...
    def generate_report(self):
        """
        @brief Генерация финансового отчета.
        """
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Ошибка", f"Не удалось сформировать отчет: {str(e)}"
            )

    def export_to_pdf(self):
        """
        @brief Экспорт отчета в PDF (заглушка).
        @note Функция пока не реализована и будет добавлена в будущих версиях.
        """
        QMessageBox.information(
            self,
    ...
    def export_to_excel(self):
        """
        @brief Экспорт отчета в Excel (заглушка).
        @note Функция пока не реализована и будет добавлена в будущих версиях.
        """
        QMessageBox.information(
            self,
    ...
    def generate_forecast(self):
        ...
        try:
            ...
            for i in range(periods):
                next_month = last_month.replace(
                    month=last_month.month % 12 + 1,
                    year=last_month.year + (last_month.month // 12),
                )
                forecast_months.append(next_month.strftime("%Y-%m"))
                self.forecast_table.setItem(
                    len(months) + i, 0, QTableWidgetItem(next_month.strftime("%Y-%m"))
                )
                self.forecast_table.setItem(
            ...
            self.forecast_table.setItem(
                ...
                5,
                QTableWidgetItem(
                    f"{mean_rel_error:.2f} ({Forecast.interpret_accuracy(mean_rel_error)})"
                ),
            )
            ...
            line_actual = ax.plot(
                months, values, label="Фактические", marker="o", color="#0000FF"
            )
            # Рисуем линию прогноза (красный цвет, пунктир)
            line_forecast = ax.plot(
                forecast_months,
                predicted_values,
                label="Прогноз",
                marker="x",
            ...
            logging.info(
                f"Цвет линии фактических данных в AdminDashboard: {line_actual[0].get_color()}"
            )
            logging.info(
                f"Цвет линии прогноза в AdminDashboard: {line_forecast[0].get_color()}"
            )
    ...
    def show_reports(self):
        """
        @brief Отображение раздела финансовых отчетов.
        """
